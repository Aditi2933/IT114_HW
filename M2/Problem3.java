...
public class Problem3 extends BaseClass {
    ...
    private static void bePositive(Object[] arr, int arrayNumber) {
        ...
        for (int i = 0; i < arr.length; i++)
        {
            ...
            {
                output[i] = Math.abs((Integer) arr[i]); // convert Integer to positive
            }
            else if (arr[i] instanceof Double)
            {
                output[i] = Math.abs((Double) arr[i]); // convert Double to positive
            }
            else if (arr[i] instanceof Float)
            {
                output[i] = Math.abs((Float) arr[i]); // convert Float to positive
            }
            else if (arr[i] instanceof String)
            {
                try
                {
                    double value = Double.parseDouble((String) arr[i]);
                    output[i] = String.valueOf(Math.abs(value)); // keep as String but positive
                }
